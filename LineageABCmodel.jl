...
function runmultiplelineageABCmodels( lineagetree::Lineagetree, nochains::UInt, model::UInt,timeunit::Float64,tempering::String, comment::String,timestamp::DateTime, MCstart::UInt,burnin::UInt,MCmax::UInt,subsample::UInt, state_init::Lineagestate2,pars_stps::Array{Float64,1}, nomothersamples::UInt,nomotherburnin::UInt, nolevels::UInt64,notreeparticles::UInt64,auxiliaryfoldertrunkname::String,useRAM::Bool,withCUDA::Bool,trickycells::Array{UInt64,1}, without::Int64,withwriteoutputtext::Bool )
    ...
    while( keeptrying )
        ...
        for j_chain = 1:nochains
            ...
            (state_init_here, _,_, dthdivdistr_here, uppars_chains[j_chain]) = ABCinitialiseLineageMCmodel( lineagetree, model,timeunit,tempering, comment,chaincomment_here,timestamp, MCstart,burnin,MCmax,subsample, state_init,pars_stps, nomothersamples,nomotherburnin, without,withwriteoutputtext )
...
function runlineageABCmodel( lineagetree::Lineagetree, state_init::Lineagestate2, nolevels::UInt64,notreeparticles::UInt64,auxiliaryfoldertrunkname::String,useRAM::Bool,withCUDA::Bool,trickycells::Array{UInt64,1}, dthdivdistr::DthDivdistr, uppars::Uppars2 )
    ...
    orderedcellweights_cs_dth .+= priorweight_here; orderedcellweights_cs_div .+= priorweight_here  # update all cellweights by the weight of the prior
    if( nolevels==0 )                                                           # indicates to set number of levels automatically
        ...
        #@printf( " (%s) Info - runlineageABCmodel (%d): Set nolevels to %d.\n", uppars.chaincomment,uppars.MCit, nolevels )
    ...
    if( uppars.without>=0 )
        ...
        if( uppars.subsample==0 )       # i.e. automatically select subsampling
            @printf( " (normal: %d, tricky: %d)\n", ceil(UInt64, -log(minimum(noparticles_lev))/log( mean(reasonablerejrange)+2*(reasonablerejrange[2]-mean(reasonablerejrange)) )), ceil(UInt64, -log(maximum(noparticles_lev))/log( mean(reasonablerejrange)+2*(reasonablerejrange[2]-mean(reasonablerejrange)) )) )
    ...
    while( j_lev>=2 )
        ...
        if( effsamplesize>=1 )                                              # some particles left
            ...
            # perturb state:
            if( useRAM )                                                    # safe all model parameters in workspace
                getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],cellorder,noparticles_lev[j_lev], state_curr_par,myABCnuisanceparameters_curr_par,logprob_curr_par,logprior_curr_par,logdthprob_curr_par, stepsize,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars )   # withunknownmothers = true
            else                                                            # safe model parameters to external files
                getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],noparticles_lev[j_lev], nuisancefullfilename,cellorder,logprob_curr_par, stepsize,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here,cellorder_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars ) # withunknownmothers = true
            end     # end if useRAM
            ...
            if( uppars_lev[j_lev].without>=1 )
                ...
                if( all( ((rejected_glob_lev[:,:,j_lev]./samplecounter_glob_lev[:,:,j_lev])[samplecounter_glob_lev[:,:,j_lev].>0]).>0.9 ) )
                    @printf( " (%s) Info - runLineageABCmodel (%d): Got almost all perturbations rejected (after %1.3f sec).\n", uppars_lev[j_lev].chaincomment,uppars_lev[j_lev].MCit, (DateTime(now())-uppars.timestamp)/Millisecond(1000) ); flush(stdout)
                    without_mem = deepcopy( uppars_lev[j_lev].without ); without_mem2 = deepcopy(uppars.without)
                    uppars_lev[j_lev].without = 4                                   # make sure to give all output
                    uppars.without = 4
                    j_par = 1
                    ...
                    myABCnuisanceparameters_curr_here[1].nocellssofar = UInt64(0)
                    @printf( " (%s) Info - runlineageABCmodel (%d): updatenuisanceparameters_cont output for j_par = %d, nocellssofar = %d, resample final parameters:\n", uppars_lev[j_lev].chaincomment,uppars_lev[j_lev].MCit, j_par, myABCnuisanceparameters_curr_here[1].nocellssofar ); flush(stdout)
                    updatenuisanceparameters_cont( lineagetree,nocells_lev[j_lev], state_curr_here[1],myABCnuisanceparameters_curr_here[1], logdthprob_curr_here[1], statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA, uppars_lev[j_lev] )  # withunknownmothers = true
                    # ....output perturbations:
                    noparticles_here::UInt64 = UInt64(1)#deepcopy(noparticles_lev[j_lev])   # only do update for first particle
                    @printf( " (%s) Info - runlineageABCmodel (%d): Cellwise contribution, stepsize %1.5e (withCUDA %d)(without %d):\n", uppars_lev[j_lev].chaincomment,uppars_lev[j_lev].MCit, stepsize, withCUDA, uppars_lev[j_lev].without )
                    if( useRAM )                                                    # safe all model parameters in workspace
                        getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],cellorder,noparticles_here, state_curr_par,myABCnuisanceparameters_curr_par,logprob_curr_par,logprior_curr_par,logdthprob_curr_par, stepsize,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars ) # withunknownmothers = true
                    else                                                            # safe model parameters to external files
                        getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],noparticles_here, nuisancefullfilename,cellorder,logprob_curr_par, stepsize,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here,cellorder_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars ) # withunknownmothers = true
                    end     # end if useRAM
                    @printf( " (%s) Info - runlineageABCmodel (%d): Cellwise contribution stepsize %1.5e (withCUDA %d)(without %d):\n", uppars_lev[j_lev].chaincomment,uppars_lev[j_lev].MCit, 0.0, withCUDA, uppars_lev[j_lev].without )
                    if( useRAM )                                                    # safe all model parameters in workspace
                        getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],cellorder,noparticles_here, state_curr_par,myABCnuisanceparameters_curr_par,logprob_curr_par,logprior_curr_par,logdthprob_curr_par, 0.0,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars )  # withunknownmothers = true
                    else                                                            # safe model parameters to external files
                        getrwperturbation_withsave_loop( lineagetree,nocells_lev[j_lev],noparticles_here, nuisancefullfilename,cellorder,logprob_curr_par, 0.0,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, state_curr_here, myABCnuisanceparameters_curr_here,logprob_curr_here,logprior_curr_here,logdthprob_curr_here,cellorder_here, statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,true,withCUDA,j_lev, reasonablerejrange, uppars_lev[j_lev], uppars )    # withunknownmothers = true
                    end     # end if useRAM
                    uppars_lev[j_lev].without = deepcopy( without_mem )             # reset without
...
function updatenuisanceparameters_cont( lineagetree_here::Lineagetree,maxcells_here::UInt64, state_here::Lineagestate2,myABCnuisanceparameters::ABCnuisanceparameters, logdthprob::Float64, statefunctions::Statefunctions,targetfunctions::Targetfunctions,dthdivdistr::DthDivdistr, treeconstructionmode::UInt64,knownmothersamplemode::UInt64,withunknownmothers::Bool,withCUDA::Bool, uppars::Uppars2 )::Tuple{Float64,Int64}
    ...
    # get updated equilibrium distribution:
    if( withunknownmothers )                            # no need to update unless global parameters got changed
        #@printf( " (%s) Info - updatenuisanceparameters_cont (%d): Start with proposal of unknownmother parameters now (after %1.3f sec).\n", uppars.chaincomment,uppars.MCit, (DateTime(now())-uppars.timestamp)/Millisecond(1000) )
        (state_here,convflag) = getunknownmotherupdatefromprior( state_here,statefunctions, uppars )
        if( convflag<0 )                                # equilibration not converged
            return -Inf, -1
        end     # end if equilibrium not converged
    ...
    # get updated tree-parameters:
    #@printf( " (%s) Info - updatenuisanceparameters_cont (%d): Start with proposal of tree-parameters now          (after %1.3f sec).\n", uppars.chaincomment,uppars.MCit, (DateTime(now())-uppars.timestamp)/Millisecond(1000) )
    # get other parameters:
    if( treeconstructionmode==1 )               # only correct tree architecture
        @printf( " (%s) Warning - updatenuisanceparameters_cont (%d): treeconstructionmode %d not implemented.", uppars.chaincomment,uppars.MCit, treeconstructionmode )
    elseif( treeconstructionmode==2 )           # correct tree architecture and timings (within measurement error)
        @printf( " (%s) Warning - updatenuisanceparameters_cont (%d): treeconstructionmode %d not implemented.", uppars.chaincomment,uppars.MCit, treeconstructionmode )
    elseif( treeconstructionmode==3 )           # correct tree architecture and timings (wihtin measurement error) using SMC
        ...
        @printf( " (%s) Warning - updatenuisanceparameters_cont (%d): Unknown treeconstructionmode %d.\n", uppars.chaincomment,uppars.MCit, treeconstructionmode )
    end     # end of distinguishing treeconstructionmodes

    return logprob_here, trickycell
end     # end of updatenuisanceparameters_cont function
    ...
    while( keeptrying )
        ...
        if( !keeptrying )                                   # ie if not rejected by prior already
            ...
            if( (dthdivdistr.typeno==UInt64(1)) || (dthdivdistr.typeno==UInt64(4)) )# death- and division process
                ...
                nodeaths_here = nodeaths + 0; nodivs_here = nodivs + 0  # use slight bias to moderate/middle values
...
function getrwperturbation_withsave_loop( lineagetree::Lineagetree,nocells_here::UInt64,noparticles_here::UInt64, nuisancefullfilename::Function,cellorder::Array{UInt64,1},logprob_curr_par::Array{Float64,1}, stepsize::Float64,myconstd::Array{Float64,1},mypwup::Array{Float64,3}, samplecounter_glob_lev::Array{UInt64,3},rejected_glob_lev::Array{Float64,3}, myABCnuisanceparameters_buff::ABCnuisanceparameters, state_curr_here::Array{Lineagestate2,1}, myABCnuisanceparameters_curr_here::Array{ABCnuisanceparameters,1},logprob_curr_here::Array{Float64,1},logprior_curr_here::Array{Float64,1},logdthprob_curr_here::Array{Float64,1},cellorder_here::Array{Array{UInt64,1},1}, statefunctions::Statefunctions,targetfunctions::Targetfunctions,dthdivdistr::DthDivdistr, treeconstructionmode::UInt64,knownmothersamplemode::UInt64,withunknownmothers::Bool,withCUDA::Bool,j_lev::UInt64, reasonablerejrange::Array{Float64,1}, uppars_here::Uppars2, uppars::Uppars2 )::Nothing
    ...
    if( withCUDA && (nocells_here>1) && (uppars_here.nohide>1) )    # 2dRW models with large memory-consumption
        for j_par = 1:noparticles_here                  # sequential particles loop
            getrwperturbation_withsave( lineagetree,nocells_here,noparticles_here, nuisancefullfilename(j_par),cellorder,view(logprob_curr_par,j_par), stepsize,myconstd,mypwup, samplecounter_glob_lev,rejected_glob_lev, myABCnuisanceparameters_buff, view(state_curr_here,Threads.threadid()),view(myABCnuisanceparameters_curr_here,Threads.threadid()),view(logprob_curr_here,Threads.threadid()),view(logprior_curr_here,Threads.threadid()),view(logdthprob_curr_here,Threads.threadid()),view(cellorder_here,Threads.threadid()), statefunctions,targetfunctions,dthdivdistr, treeconstructionmode,knownmothersamplemode,withunknownmothers,withCUDA,j_lev, reasonablerejrange,  uppars_here, uppars )
        end     # end of particles loop
    else                                                # nothing to hide
        #for j_par = 1:noparticles_here
        Threads.@threads for j_par = 1:noparticles_here
...
function getrwperturbation( lineagetree::Lineagetree,nocells_here::UInt64,cellorder::Array{UInt64,1},noparticles_here::UInt64, stepsize::Float64,myconstd::Array{Float64,1},mypwup::Array{Float64,3}, samplecounter_glob_lev::Array{UInt64,3},rejected_glob_lev::Array{Float64,3}, myABCnuisanceparameters_buff::ABCnuisanceparameters, state_curr_here::SubArray{Lineagestate2,0}, myABCnuisanceparameters_curr_here::SubArray{ABCnuisanceparameters,0},logprob_curr_here::SubArray{Float64,0},logprior_curr_here::SubArray{Float64,0},logdthprob_curr_here::SubArray{Float64,0}, statefunctions::Statefunctions,targetfunctions::Targetfunctions,dthdivdistr::DthDivdistr, treeconstructionmode::UInt64,knownmothersamplemode::UInt64,withunknownmothers::Bool,withCUDA::Bool,j_lev::UInt64, reasonablerejrange::Array{Float64,1}, uppars_here::Uppars2 )::Nothing
            ...
            if( (logprior_prop_here+loghastings)>-Inf )             # otherwise nothing to do
                ...
                if( log(rand())<logaccprob )     # accept
                    if( uppars_here.without>=3 )
                        @printf( " (%s) Info - getrwperturbation (%d): Accept   (j_rep=%2d,j_up=%2d) proposal, threadid %2d/%2d (logrelprob = %+1.5e)(after %1.3f sec).\n", uppars_here.chaincomment,uppars_here.MCit, j_rep,j_up, Threads.threadid(),Threads.nthreads(), (logprob_prop_here-logprob_curr_here[1]) + (logprior_prop_here-logprior_curr_here[1]), (DateTime(now())-uppars_here.timestamp)/Millisecond(1000) ); flush(stdout)
                    ...
                    if( uppars_here.without>=3 )
                        @printf( " (%s) Info - getrwperturbation (%d): Rejected (j_rep=%2d,j_up=%2d) proposal, threadid %2d/%2d (logaccprob = %+1.5e,  logrellklh = %+1.5e, logrelprior = %+1.5e, loghastings = %+1.5e)(after %1.3f sec).\n", uppars_here.chaincomment,uppars_here.MCit, j_rep,j_up, Threads.threadid(),Threads.nthreads(), logaccprob, (logprob_prop_here-logprob_curr_here[1]), (logprior_prop_here-logprior_curr_here[1]), loghastings, (DateTime(now())-uppars_here.timestamp)/Millisecond(1000) ); flush(stdout)
                        #@printf( " (%s)  loglklh_prop = %+1.5e, logprior_prop = %+1.5e, loghastings = %+1.5e (threadid %2d/%2d).\n", uppars_here.chaincomment, logprob_prop_here,logprior_prop_here,loghastings, Threads.threadid(),Threads.nthreads() )
                        @printf( " (%s)  pars_curr =     [ %s] (threadid %2d/%2d).\n", uppars_here.chaincomment, join([@sprintf("%+1.10e ",j) for j in state_curr_here[1].pars_glob]), Threads.threadid(),Threads.nthreads() )
                        @printf( " (%s)  pars_prop =     [ %s] (threadid %2d/%2d).\n", uppars_here.chaincomment, join([@sprintf("%+1.10e ",j) for j in state_prop_here.pars_glob]), Threads.threadid(),Threads.nthreads() )
                        #pars_glob_new = zeros(Float64, uppars_here.noglobpars)  # allocate
                        #getoldtonewparameters( state_curr_here[1].pars_glob,pars_glob_new, uppars_here )
                        ...
                        #getoldtonewparameters( state_prop_here.pars_glob,pars_glob_new, uppars_here )
                ...
                if( uppars_here.without>=3 )
                    @printf( " (%s) Info - getrwperturbation (%d): Directly rejected (j_rep=%2d,j_up=%2d) proposal, threadid %2d/%2d (logrelprior = %+1.5e, loghastings = %+1.5e)(after %1.3f sec).\n", uppars_here.chaincomment,uppars_here.MCit, j_rep,j_up, Threads.threadid(),Threads.nthreads(), (logprior_prop_here-logprior_curr_here[1]), loghastings, (DateTime(now())-uppars_here.timestamp)/Millisecond(1000) ); flush(stdout)
    ...
    #@printf( " (%s) Info - getrwperturbation (%d): Done perturbation    threadid %2d/%2d,                    gctotmem = %10.1f MB, gclivemem = %10.1f MB, jitmem = %10.1f MB, Maxrss = %10.1f MB.\n", uppars_here.chaincomment,uppars_here.MCit, Threads.threadid(),Threads.nthreads(), Base.gc_total_bytes(Base.gc_num())/2^20, Base.gc_live_bytes()/2^20, Base.jit_total_bytes()/2^20, Sys.maxrss()/2^20 ); flush(stdout)
    ...
    elseif( knownmothersamplemode==2 )                      # sampling conditioned on fate
        if( model in (11,12,13,14) )                        # GammaExponential distributed event-times
            ...
            if( !isfinite(probval_here) )                   # might be numerical problem only - switch to double precision
                probvals1_64::Float64 = CUDAloginvGammaExponential_cdf6464( pars_cell_here_part, Float64(xbounds_here_part[1]), cellfate )
                probvals2_64::Float64 = CUDAloginvGammaExponential_cdf6464( pars_cell_here_part, Float64(xbounds_here_part[2]), cellfate )
                probval_here_64::Float64 = logsubexp(probvals1_64,probvals2_64)
                #@cuprintf( " Warning -CUDAgetknownmotherpropagation: Single precision probvals = %+1.5e (%+1.5e, %+1.5e);  double-precision probvals = %+1.5e (%+1.5e, %+1.5e).\n", probval_here,probvals1,probvals2, probval_here_64, probvals1_64, probvals2_64 )
        ...
        if( all(myABCnuisanceparameters.particlelogweights[cell_here,:].==-Inf) )
            ...
            if( uppars.without>=3 )
                ...
                if( effsamplesize<effsamplesizethreshold )
                    @printf( " (%s) Info - getcorrecttreewithinerrorsSMCproposal_cont (%d): Only %4.1f < %4d in [%d..%d] effective samples, logprob = %+1.5e, for cell=%d, threadid=%2d/%2d (mother=%d,cellfate=%d)(meanfate=%1.3f, pars_cell_mean=[ %s]). logweights = [%+1.3e..%+1.3e], %+1.3e+-%1.3e, noinfs = %d/%d.\n", uppars.chaincomment,uppars.MCit, effsamplesize,effsamplesizethreshold,1,noparticles,logprob_here, cell_here, Threads.threadid(),Threads.nthreads(), mother,cellfate, mean(myABCnuisanceparameters.fates_cell_part[cell_here,:]), join([@sprintf("%+1.3e ",mean(myABCnuisanceparameters.pars_cell_part[cell_here,j,:])) for j in 1:uppars.nolocpars]), minimum(myABCnuisanceparameters.particlelogweights[cell_here,:]),maximum(myABCnuisanceparameters.particlelogweights[cell_here,:]), mean(myABCnuisanceparameters.particlelogweights[cell_here,:]),std(myABCnuisanceparameters.particlelogweights[cell_here,:]), sum(myABCnuisanceparameters.particlelogweights[cell_here,:].==-Inf),noparticles )
                else
                    if( uppars.without>=3 )
                        @printf( " (%s) Info - getcorrecttreewithinerrorsSMCproposal_cont (%d): Got  %4d < %4.1f in [%d..%d] effective samples, logprob = %+1.5e, for cell=%d, threadid=%2d/%2d (mother=%d,cellfate=%d)(meanfate=%1.3f, pars_cell_mean=[ %s]).\n", uppars.chaincomment,uppars.MCit, effsamplesizethreshold,effsamplesize,1,noparticles,logprob_here, cell_here, Threads.threadid(),Threads.nthreads(), mother,cellfate, mean(myABCnuisanceparameters.fates_cell_part[cell_here,:]), join([@sprintf("%+1.3e ",mean(myABCnuisanceparameters.pars_cell_part[cell_here,j,:])) for j in 1:uppars.nolocpars]) )
                    end     # end if without
                end     # end if too few samples
            end     # end if without
        end     # end if unhealthy particle weights
...
function getweightedcellnumber( lineagetree::Lineagetree,trickycells::Array{UInt64,1}, statefunctions::Statefunctions,targetfunctions::Targetfunctions,dthdivdistr::DthDivdistr, knownmothersamplemode::UInt64,withCUDA::Bool, uppars::Uppars2 )::Tuple{Array{Float64,1},Array{UInt64,1},Array{UInt64,1}}
    ...
    myABCnuisanceparameters_here::ABCnuisanceparameters = ABCnuisanceparameters(uppars.nocells,notreeparticles, zeros(uppars.nocells,notreeparticles), zeros(UInt64,uppars.nocells,notreeparticles), zeros(uppars.nocells,uppars.nohide,notreeparticles), zeros(uppars.nocells,uppars.nolocpars,notreeparticles), zeros(uppars.nocells,2,notreeparticles), zeros(UInt64,uppars.nocells,notreeparticles) )      # allocate memory
    cellorder::Array{UInt64,1} = getcorrecttreewithinerrorsSMCproposal_cont( lineagetree, lineagetree.nocells, state_here,myABCnuisanceparameters_here, 0.0,  statefunctions,targetfunctions,dthdivdistr, knownmothersamplemode,withCUDA, uppars )[2]
...
function getcondvariance( state_curr_par::Array{Lineagestate2,1}, logrelativeweight::Array{Float64,1}, effsamplesize::Float64, withreparametrisation::Bool, uppars::Uppars2 )::Tuple{Array{Float64,1}, Array{Float64,3}, Array{Float64,2}, Array{Float64,1}, UInt64}
    ...
    try
        evals = eigvals(mycov); mineval = minimum(evals)#; display(evals)
    catch error_here
    ...
    #@printf( " (%s) Info - getcondvariance (%d): New mean = [ %s], new var = [ %s].\n", uppars.chaincomment,uppars.MCit, join([@sprintf("%+1.5e ",j) for j in mymean]), join([@sprintf("%+1.5e ",mycov[j,j]) for j in 1:uppars.noglobpars]) )
    ...
    if( allsamplescounter>=minsamples )
        ...
        if( uppars.without>=0 )
            ...
            if( stepsize>=stepsizemin )
                ...
                @printf( " (%s) Info - adjuststepsizes (%d): Adjust update, %1.5e --> %1.5e --> %1.5e\t(rej %1.3f)\n", uppars.chaincomment,uppars.MCit, oldsteps,stepsize,stepsizemin, rejrate )
...
function analysemultipleABCstatistics( lineagetree::Lineagetree, state_chains_hist::Array{Array{Lineagestate2,1},1}, logprob_chains::Array{Float64,1}, uppars_chains::Array{Uppars2,1}, withgraphical::Bool,withrotationcorrection::Bool=true )::Nothing
    ...
    if( mymodel in (4,9,14) )               # also output eigenvalue statistics, in case of hiddenmatrix models
        ...
        dthdivdistr::DthDivdistr = getstateandtargetfunctions( mymodel )[3]
        for j_prs in eachindex(state_prs_hist)
            ...
            while( logprior==-Inf )
                ...
                logprior = getjointpriorrejection( pars_glob_prs, dthdivdistr, uppars_chains[1] )   # reject wrong eigenvalues
        ...
        (eigenvalues_prs_hist,categories_prs_hist) = ABCgeteigenvaluehist( state_prs_hist, uppars_chains[1] )
    ...
    for j_globpar = 1:mynoglobpars
        ...
        if( dimension_here=="time" )        # time
            ...
            unitsymbol_here = "h"; unitrefforgraphs_here = " (h)"
        elseif( dimension_here=="angle" )   # angle
            unit_here = 180/pi
            unitsymbol_here = "deg"; unitrefforgraphs_here = " (deg)"
    ...
    if( mymodel in (4,9,14) )                           # also output eigenvalue statistics, in case of hiddenmatrix models
        ...
        cat12eigenvalues::Array{Float64,2} = zeros(2,nochains*nosamples); cat3eigenvalues::Array{Float64,2} = zeros(2,nochains*nosamples)
        cat12eigenvalues_prs::Array{Float64,2} = zeros(2,nopriorsamples); cat3eigenvalues_prs::Array{Float64,2} = zeros(2,nopriorsamples)
        ...
        for j_chain = 1:nochains
            for j_sample = 1:nosamples
                ...
                elseif( categories_chains_hist[j_chain,j_sample]==3 )
                    ...
                    @printf( " Info - analysemultipleABCstatistics: Unknown category %d for chain %d, sample %d.\n", eigenvalues_chains_hist[j_chain,j_sample], j_chain,j_sample )
        ...
        for j_prs = 1:nopriorsamples
            ...
            elseif( categories_prs_hist[j_prs]==3 )
                ...
                @printf( " Info - analysemultipleABCstatistics: Unknown category %d for priorsample %d.\n", categories_prs_hist[j_prs], j_prs )
            end     # end of distinguishing categories
        end     # end of priorsamples loop
        @printf( " Info - analysemultipleABCstatistics: Eigenvalue statistics:\n" )
        selectcat_here::Array{Bool,1} = .!isnan.(cat12eigenvalues[1,:])
        @printf( "  real  eigenvalues: smaller %+12.5e +- %11.5e, larger %+12.5e +- %11.5e\n", mean(cat12eigenvalues[1,selectcat_here]),std(cat12eigenvalues[1,selectcat_here]), mean(cat12eigenvalues[2,selectcat_here]),std(cat12eigenvalues[2,selectcat_here]) )
        selectcat_here .= .!isnan.(cat3eigenvalues[1,:])
        @printf( "  cmplx eigenvalues: real    %+12.5e +- %11.5e, imag   %+12.5e +- %11.5e\n", mean(cat3eigenvalues[1,selectcat_here]),std(cat3eigenvalues[1,selectcat_here]), mean(cat3eigenvalues[2,selectcat_here]),std(cat3eigenvalues[2,selectcat_here]) )
        ...
        if( withgraphical )
            ...
            histogram!( largestabseigenvalues, color=mycolours(1), label="inference", lw=0 )
            plot!( mysubbins_here, mypriorvalues.*(dvalue*nochains*nosamples), color=mycolours(0), label="prior",lw=2 )
            ...
            histogram!( phaseofeigenvalues.*(180/pi), color=mycolours(1), label="inference", lw=0 )
            plot!( mysubbins_here.*(180/pi), mypriorvalues.*(dvalue*nochains*nosamples), color=mycolours(0), label="prior",lw=2 )
            if( 0<mysubbins_here[1]<=dvalue_prs )          # add peak at zero
                plot!( [0.0,mysubbins_here[1]*(180/pi)], [categories_prs[1]*(nochains*nosamples),mypriorvalues[1]*(dvalue*nochains*nosamples)], colour=mycolours(0), label="",lw=2 )
            ...
            minvalue = minimum(phaseofeigenvalues)*(180/pi); maxvalue = maximum(phaseofeigenvalues)*(180/pi)
            dvalue = max(1E-10,(maxvalue-minvalue)/myres_stats_all)
            if( minvalue<=dvalue )
                minvalue = 0.0
            end     # end if close to zero
    ...
    if( withgraphical )
        if( (mymodel==1) || (mymodel==2) || (mymodel==3) || (mymodel==4) || (mymodel==9) || (mymodel==11) || (mymodel==12) || (mymodel==13) || (mymodel==14) )  # only meaningful, if a model with rescaled time
            ...
            minvalue = minimum(lifetimes[(-Inf.<lifetimes.<+Inf).&&(.!isnan.(lifetimes))])*unit_here; maxvalue = maximum(lifetimes[(-Inf.<lifetimes.<+Inf).&&(.!isnan.(lifetimes))])*unit_here
            ...
            histogram!( (lifetimes[:,mystatsrange,fateandcensoring.==1].*unit_here)[:], bins=mybins_here, label="", color=mycolours(2), opacity=0.5, lw=0 )
            ...
            histogram!( 1.0 .-exp.(target_cdf_hist[:,mystatsrange,fateandcensoring.==1])[:], bins=mybins_here, label="", color=mycolours(2), opacity=0.5, lw=0 )
            histogram!( 1.0 .-exp.(target_cdf_hist[:,mystatsrange,fateandcensoring.==2])[:], bins=mybins_here, label="", color=mycolours(1), opacity=0.5, lw=0 )
    ...
    (mycov::Array{Float64,2},coverrorflag::UInt64) = getcondvariance( state_list, ones(nochains*length(mystatsrange)), -1.0, false, uppars_chains[1] )[[3,5]]  # without reparametrisation
    if( coverrorflag==0 )                   # ie no error
        @printf( " Info - analysemultipleABCstatistics: Got posterior correlations of global parameters:\n" )
    ...
    (mycov,coverrorflag) = getcondvariance( state_list, ones(nochains*length(mystatsrange)), -1.0, true, uppars_chains[1] )[[3,5]]  # without reparametrisation
    if( coverrorflag==0 )                   # ie no error
        @printf( " Info - analysemultipleABCstatistics: Got posterior correlations of global parameters: (after reparametrisation)\n" )
        @printf( "       %s\n", join([@sprintf("%5d  ",j) for j in 1:mynoglobpars]) )
        for j_globpar = 1:mynoglobpars
            #@printf( "  %2d:  %s\n", j_globpar, join([@sprintf("%+5.2f  ",mycov[j_globpar,j]/sqrt(mycov[j_globpar,j_globpar]*mycov[j,j])) for j in 1:mynoglobpars]) )
            @printf( "  %2d:  ", j_globpar )
    ...
    if( withgraphical && (mymodel in (4,9,14)) )    # also output eigenvalue statistics, in case of hiddenmatrix models
        ...
        for j_test in eachindex(state_list)
            ...
            getoldtonewparameters( pars_glob_old_here,view(pars_glob_new_here, :), uppars_chains[1] )
            getnewtooldparameters( view(pars_glob_buffer_here, :),pars_glob_new_here, uppars_chains[1] )
            if( any(abs.(pars_glob_old_here.-pars_glob_buffer_here).>1e-10) )
                hiddenmatrix = zeros(2,2); hiddenmatrix[:] .= pars_glob_old_here[4:7]
                @printf( " Warning - analysemultipleABCstatistics: Test %d gives max-deviation %1.5e (tr = %+1.5e, det = %+1.5e, a11-a22 = %+1.5e).\n", j_test, maximum(abs.(pars_glob_old_here.-pars_glob_buffer_here)), tr(hiddenmatrix),det(hiddenmatrix), hiddenmatrix[1,1]-hiddenmatrix[2,2] )
                display( pars_glob_old_here' ); hiddenmatrix[:] .= pars_glob_old_here[4:7]; display(eigen(hiddenmatrix))
                display( pars_glob_buffer_here' ); hiddenmatrix[:] .= pars_glob_buffer_here[4:7]; display(eigen(hiddenmatrix))
            end     # end if deviates
        ...
        rejcounter1::Int64 = 0; rejcounter2::Int64 = 0; before_here::Array{Float64,1}= Array{Float64,1}(undef,2); outerboundary::Float64 = 6.0
        for j_sample = 2:mysamples
            #sampledindex = samplefromdiscretemeasure(-log.(weights))[1]
            ...
            getoldtonewparameters( pars_glob_old_here,view(pars_glob_new_here, :), uppars_chains[1] )
            logacceptratio = +getlogdetnewparameters( pars_glob_new_here, uppars_chains[1] )
            pars_glob_new_here[6] += 20*(2*rand()-1)
            logacceptratio -= getlogdetnewparameters( pars_glob_new_here, uppars_chains[1] )
            getnewtooldparameters( view(pars_glob_old_here, :),pars_glob_new_here, uppars_chains[1] )
...
function ABCwritelineagestatetotext( lineagetree::Lineagetree, state_hist::Array{Lineagestate2,1}, logweight_hist::Array{Float64,1},logprob::Float64, nolevels::UInt64,treeconstructionmode::UInt64, noparticles_lev::Array{UInt64,1},temperature_lev::Array{Float64,1},reasonablerejrange::Array{Float64,1}, uppars::Uppars2 )::Nothing
    ...
    open( uppars.outputfile, "w" ) do myfile
        ...
        @printf( " (%s)  subsample:   %d", uppars.chaincomment, uppars.subsample )
        if( uppars.subsample==0 )       # i.e. automatically select subsampling
            write( myfile, @sprintf(" (normal: %d, tricky: %d)\n", ceil(UInt64, -log(minimum(noparticles_lev))/log( mean(reasonablerejrange)+2*(reasonablerejrange[2]-mean(reasonablerejrange)) )), ceil(UInt64, -log(maximum(noparticles_lev))/log( mean(reasonablerejrange)+2*(reasonablerejrange[2]-mean(reasonablerejrange)))) ) )
        else    # automatically select subsampling
            write( myfile, "\n" ) # just finish this line
        end     # end if automatically select subsampling
...
function ABCreadlineagestatefromtext( fullfilename::String )::Tuple{Array{Lineagestate2,1},Array{Float64,1},Float64,Uppars2,Lineagetree}
    ...
    open( fullfilename ) do myfile
        ...
        whatitis = findfirst( "subsample:   \t",newline ); whatitisend = findfirst( " (", newline )    # check opening bracket of extra "(normal: %d, tricky: %d)" info
...
function mycolours( j_graph::Int64 )::Union{String,PlotUtils.ContinuousColorGradient}
    ...
    if( j_graph==-1 )           # cyan to dark-blue gradient
        #mycolour = cgrad( [mycolours(3), mycolours(1), mycolours(4)], scale = :exp )  # gradient
        mycolour = cgrad( [mycolours(3), mycolours(1), "white"], scale = :exp )  # gradient
    elseif( j_graph==0 )
    ...
    elseif( j_graph==4 )
        ...
        #@printf( " Info - mycolours: Not implemented for %d graphs. Use dimmed version of other colours.\n", j_graph )
        ...
        buffer_hsi::HSI{Float32} = parse(HSI{Float32}, mycolours(myrem) )

...
const ALLOWED_MODELS = Dict{String, UInt64}(
	"perfect_FW" => 1, # simple Frechet-Weibull model with global parameters
	"clock_FW" => 2, # clock-modulated Frechet-Weibull model
	"RW_FW" => 3, # Frechet-Weibull model with random walk inheritance
	"2DRW_FW" => 4, # Frechet-Weibull model with 2D random walk inheritance
	"2DRW_F" => 9, # Frechet-Weibull model with 2D random walk inheritance, divisions-only
	"perfect_GE" => 11, # simple Gamma-Exponetial model with global parameters
	"clock_GE" => 12, # clock-modulated Gamma-Exponetial model
	"RW_GE" => 13, # Gamma-Exponetial model with random walk inheritance
	"2DRW_GE" => 14, # Gamma-Exponetial model with 2D random walk inheritance
)
...
function control_get_lineage_abc_dynamics(;
	...
	@printf(" Info - control_get_lineage_abc_dynamics: Start now %1.3f sec\n", (DateTime(now()) - t1) / Millisecond(1000))
	flush(stdout)
	...
	if (!isempty(filename))                        # read existing file, if filename is meaningful
		(fullfilename, lineagedata) = read_lineage_file(trunkfilename, filename)
		unknownfates = -1
		mylineagetree = initialise_lineage_tree(fullfilename, lineagedata, unknownfates, missingframes)
		...
		model2_sim::UInt64 = UInt64(14)
		pars_glob2_sim::Array{Float64, 1} = vcat(pars_glob_sim, [0.25, +0.65, -0.7, +0.95, 0.2, 0.1])   # inhmatrix, sigma1,sigma2 extra
		hiddenmatrix::Array{Float64, 2} = zeros(2, 2)
		hiddenmatrix[:] = pars_glob2_sim[4 .+ collect(1:4)]
		eigenstruct = eigen(hiddenmatrix)
		@printf(" Info - control_get_lineage_abc_dynamics: eigenvalues: %+1.3f%+1.3fi, %+1.3f%+1.3fi.\n", real(eigenstruct.values[1]), imag(eigenstruct.values[1]), real(eigenstruct.values[2]), imag(eigenstruct.values[2]))
		(mylineagetree, datawd) = simulate_lineage_tree_2(pars_glob2_sim, model2_sim, nobranches_sim, minnocells_sim, Int64(without_sim))[[1, 2]]
		outputfilename = write_lineage_tree(mylineagetree, mylineagetree.name)
		if ((model2_sim == 1) | (model2_sim == 2) | (model2_sim == 3) | (model2_sim == 4))  # Frechet-Weibull models
			(mean_div, std_div, mean_dth, std_dth, prob_dth) = estimate_frechet_weibull_stats(pars_glob_sim)
		elseif ((model2_sim == 11) | (model2_sim == 12) | (model2_sim == 13) | (model2_sim == 14))   # Gamma-Exponential model2_sim
			(mean_div, std_div, mean_dth, std_dth, prob_dth) = estimate_gamma_exponential_stats(pars_glob_sim)
		else                                        # unknown
			@printf(" Warning - control_get_lineage_abc_dynamics: Unknown model %d.\n", model_2sim)
		end     # end of distinguishing models
		@printf(" Info - control_get_lineage_abc_dynamics: Stats from birth: div = %1.5e+-%1.5e, dth = %1.5e+-%1.5e, prob_dth = %1.5e.\n", mean_div, std_div, mean_dth, std_dth, prob_dth)
	end     # end if read or simulate
	nocells::UInt64 = mylineagetree.nocells         # number of cells in lineagetree data
	nogenpercell::Array{UInt64, 1} = get_number_of_generations(mylineagetree)
	maxgen::UInt64 = maximum(nogenpercell)
	nounknownmothercells::UInt64 = sum(mylineagetree.datawd[:, 4] .<= 0)   # number of cells with unknown mother
	@printf(" Info - control_get_lineage_abc_dynamics: Number of cells in dataset: %d, number of generations: %d (%d times), cells with unknown mother: %d.\n", Int64(nocells), Int64(maxgen), sum(nogenpercell .== maxgen), nounknownmothercells)
	flush(stdout)
	...
	(noups::UInt64, noglobpars::UInt64, nohide::UInt64, nolocpars::UInt64) = get_mc_model_number_updates_2(model, nocells)
	...
	run_multiple_lineage_abc_models(
		mylineagetree,
		nochains,
		model,
		timeresolution,
	...
	@printf(" Info - control_get_lineage_abc_dynamics: Done now %1.3f sec.\n", (DateTime(now()) - t1) / Millisecond(1000))
end   # end of control_get_lineage_abc_dynamics function

function get_cross_generation_statistics(lineagetree::Lineagetree, depth::UInt64 = UInt64(4), without::Bool = true, withgraphical::Bool = true)
	...
	Cr::Array{Float64, 2} = zeros(depth + 1, depth + 1)
	p_cr::Array{Float64, 2} = zeros(depth + 1, depth + 1)
	for j_cell1 ∈ 1:nocells
		for j_cell2 ∈ j_cell1:nocells
			...
			if ((min(nogen1, nogen2) >= 0) & (max(nogen1, nogen2) <= depth))
				if ((get_mother(lineagetree, Int64(j_cell1))[2] > 0) & (get_mother(lineagetree, Int64(j_cell2))[2] > 0))               # must have mother (not unknown)
					if ((get_life_data(lineagetree, Int64(j_cell1))[2] == 2) & (get_life_data(lineagetree, Int64(j_cell2))[2] == 2)) # must divide (not die or unknown)
						...
						if (indexcounter[nogen1+1, nogen2+1] > size(lifetimes, 4)) # allocated memory too small
							...
							@printf(" Info - get_cross_generation_statistics: Added %d datapairs to lifetimes and listids to get total of %d.\n", noadd, size(lifetimes, 4))
						end     # end if allocated memory insufficient
						lifetimes[nogen1+1, nogen2+1, 1, indexcounter[nogen1+1, nogen2+1]] = get_life_data(lineagetree, Int64(j_cell1))[1]
						lifetimes[nogen1+1, nogen2+1, 2, indexcounter[nogen1+1, nogen2+1]] = get_life_data(lineagetree, Int64(j_cell2))[1]
						...
						lifetimes[nogen2+1, nogen1+1, 1, indexcounter[nogen2+1, nogen1+1]] = get_life_data(lineagetree, Int64(j_cell2))[1]
						lifetimes[nogen2+1, nogen1+1, 2, indexcounter[nogen2+1, nogen1+1]] = get_life_data(lineagetree, Int64(j_cell1))[1]
	...
	if (without)
		@printf(" Info - get_cross_generation_statistics: Division time correlations are:\n")
	end     # end of without
	...
	for nogen1 ∈ 0:depth
		for nogen2 ∈ nogen1:depth
			j_depth1 = nogen1 + 1
			j_depth2 = nogen2 + 1
			(Cr_mean, Cr_std, p_here, C_r_here) = getcov(
				lifetimes[j_depth1, j_depth2, 1, 1:indexcounter[j_depth1, j_depth2]],
				lifetimes[j_depth1, j_depth2, 2, 1:indexcounter[j_depth1, j_depth2]],
				nogen1 == nogen2,
				without,
				withgraphical & false,
				[@sprintf("gen%d", nogen1), @sprintf("gen%d", nogen2)],
			)
			Cr[j_depth1, j_depth2] = Cr_mean
			Cr[j_depth2, j_depth1] = Cr_mean
	...
	if (without)                                           # output cousin-mother-inequality
		if (depth >= 2)                                  # does include cousins
			...
			@printf(" Info - get_cross_generation_statistics: Cousin-mother inequality %+1.5e +- %1.5e (%1.5e).\n", mean(cmineq), std(cmineq), mean(cmineq .> 0))
	...
	return Cr
end     # end of get_cross_generation_statistics function
...
function getcov(parlist1::Array{Float64, 1}, parlist2::Array{Float64, 1}, issymmetric::Bool = false, without::Bool = false, withgraphical::Bool = false, parnames::Array{String, 1} = ["par1", "par2"])
	...
	for j_sample ∈ 1:nosamples
		...
		if (issymmetric)       # symmetric case
			parlist1_here = vcat(parlist1[select], parlist2[select])
			parlist2_here = vcat(parlist2[select], parlist1[select])
			...
			parlist1_here = parlist1[select]
			parlist2_here = parlist2[select]
...
function simulate_lineage_tree_2(pars_glob::Array{Float64, 1}, model::UInt64, nobranches::UInt64, nocells::UInt64, without::Int64 = 0)
	# simulates lineagetrees with given model according to LineageMCmodel2
	if (without >= 1)
		t1 = DateTime(now())                            # for timer
		@printf(" Info - simulate_lineage_tree_2: Start simulating model %d, nobranches %d, nocells %d, pars_glob = [ %s].\n", model, nobranches, nocells, join([@sprintf("%+1.5e ", j) for j in pars_glob]))
	end     # end if without

	# set auxiliary parameters:
	noequilibriumsamples::UInt64 = nobranches * 100           # number of cells in equilibrium samples preparation
	if (nobranches > nocells)
		@printf(" Warning - simulate_lineage_tree_2: Got nobrachnes = %d, but nocells = %d.\n", nobranches, nocells)
	...
	if ((model == 1) | (model == 2) | (model == 3) | (model == 4) | (model == 9) | (model == 11) | (model == 12) | (model == 13) | (model == 14))    # models with first couple of parameters in pars_glob coinciding with global means of pars_cell
		...
		@printf(" Warning - simulate_lineage_tree_2: Model %d is not compatible for determining 'typical' pars_cell.\n", model)
	...
	nosamples_here::UInt64 = UInt64(1000)
	samples_here::Array{Float64, 1} = zeros(nosamples_here)
	...
	uppars::Uppars2 = Uppars2(
		...
		"none",
		model,
		[getFulldistributionfromparameters("cutoffGauss", [0.0, 1.0])],
		0.0,
		1.0,
		UInt64(1),
		noglobpars,
	...
	unknownmothersamples = Unknownmotherequilibriumsamples(
		uppars.unknownmotherstarttimes[1],
	...
	if (convflag != 1)
		@printf(" Warning - simulate_lineage_tree_2: Getting equilibrium parameters not converged.\n")
	end     # end if not converged
	pars_evol::Array{Float64, 2} = unknownmothersamples.pars_evol_eq
	pars_cell::Array{Float64, 2} = unknownmothersamples.pars_cell_eq
	times_cell::Array{Float64, 2} = unknownmothersamples.time_cell_eq
	fates_cell::Array{Int64, 1} = unknownmothersamples.fate_cell_eq  # short-hand
	# ...Euler-Lotka model for total growth-rate per time:
	(beta::Float64, timerange::Array{Float64, 1}) = getEulerLotkabeta(pars_cell_here, dthdivdistr)
	...
	logdthintegral::Float64 = logsumexp(logdthintegralterms)
	alpha::Float64 = 1 / (1 + 2 * exp(logdthintegral))
	...
	elseif (alpha < (1 / 2))                                   # decline, also beta<0
		if ((nocells / nobranches) <= (1 / (1 - 2 * alpha)))         # possible to find large enough totaltime
			...
			@printf(" Warning - simulate_lineage_tree_2: Got too many cells (%d) for too few branches (%d) - reset number of branches to %d (alpha = %+1.5e, beta = %+1.5e).\n", nocells, nobranches, newnobranches, alpha, beta)
			nobranches = deepcopy(newnobranches)
			totalsimulationtime = log(((nocells / nobranches) * (2 * alpha - 1) + 1) / (2 * alpha)) / beta      # exponential growth model
		end     # end if enough branches
	else                                                    # impossible alpha
		@printf(" Warning - simulate_lineage_tree_2: alpha = %+1.5e, beta = %+1.5e (pars_cell_here = [ %s]).\n", alpha, beta, join([sprintf("%+1.5e ", j) for j in pars_cell_here]))
	...
	if (without >= 1)
		@printf(
			" Info - simulate_lineage_tree_2: Estimated totalsimulationtime = %1.5e for %d cells, %d branches, meaneventtime = %1.5e (alpha = %+1.5e, beta = %+1.5e, pars_cell_here = [ %s]).\n",
			totalsimulationtime,
			nocells,
			nobranches,
			meaneventtime,
	...
	pars_evol_hist::Array{Float64, 2} = Array{Float64, 2}(undef, 0, nohide)
	pars_cell_hist::Array{Float64, 2} = Array{Float64, 2}(undef, 0, nolocpars)
	pars_evol_dght::Array{Float64, 1} = zeros(nohide)
	pars_cell_dght::Array{Float64, 1} = zeros(nolocpars)
	while (j_cell < nocells)                                 # keep adding cells until nocells many have been simulated
		...
		newpars_evol = deepcopy(pars_evol[mysample, :])
		newpars_cell = deepcopy(pars_cell[mysample, :])
		newtimes_cell = [0.0, deepcopy(times_cell[mysample, 2])]
		newfates_cell = deepcopy(fates_cell[mysample])
		...
		pars_evol_hist = cat(pars_evol_hist, newpars_evol', dims = 1)
		pars_cell_hist = cat(pars_cell_hist, newpars_cell', dims = 1)
		...
		elseif ((newfates_cell == 2) & (datawd[j_cell, 3] < totalsimulationtime))   # division of starter cell; add this cell to nextones lists
			...
			@printf(" Warning - simulate_lineage_tree_2: Unknown fate of cell %d: %d.\n", j_cell, newfates_cell)
		...
		while (!isempty(nextones_id))                      # still some dividing ones in this branch
			...
			splice!(nextones_id, 1)
			splice!(nextones_time, 1)# remove obsolete division
			...
			for daughter ∈ 1:2
				...
				pars_evol_hist = cat(pars_evol_hist, pars_evol_dght', dims = 1)
				pars_cell_hist = cat(pars_cell_hist, pars_cell_dght', dims = 1)
	...
	fullfilename::String = @sprintf(
		"%04d-%02d-%02d_%02d-%02d-%02d_Simulation2_cells=%d,model=%d,pars=[ %s].txt",
		year(timestamp),
		month(timestamp),
		day(timestamp),
		hour(timestamp),
		...
		j_cell,
		model,
		join([@sprintf("%+1.5e ", j) for j in pars_glob[:, 1]])
	)
	...
	mylineagetree::Lineagetree = initialise_lineage_tree(fullfilename, lineagedata, nothing)      # lineagetree
	if (without >= 1)
		@printf(" Info - simulate_lineage_tree_2: Done simulating %s (after %1.3f sec).\n", fullfilename, (DateTime(now()) - t1) / Millisecond(1000))
	end     # end if without

	return mylineagetree, datawd, pars_evol_hist, pars_cell_hist
end     # end of simulate_lineage_tree_2 function

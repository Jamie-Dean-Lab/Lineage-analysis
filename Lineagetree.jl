...
function initialiseLineagetree(name::String,data::Array{Int64,2}, unknownfates=-1,missingframes=-1)
    ...
    cellindices = data[:,1]
    datawd = setdatawd(data)
    firstframe = setfirstframe(data)
    lastframe = setlastframe(data)
    lastcell = setlastcell(data)
    if( unknownfates==-1 )
...
function setdatawd(data)
    ...
    for j_cell = 1:nocells
        ...
        if( data[j_cell,4]>0 )              # ie mother exists
            ...
            elseif( mydatawd[mydatawd[j_cell,7],6]<0 )      # second daughter not found yet
                ...
                @printf( " Warning - setdatawd: Got more than two daughters for mother=%d, j_cell=%d\n", mydatawd[j_cell,7], j_cell )
    ...
    #display(mydatawd)
    return mydatawd
end     # end of setdatawd function
function setfirstframe(data)
    # gets the smallest first frame number among all cells
    return minimum( data[:,2] )
end     # end of setfristframe function
function setlastframe(data)
    # gets the highest last frame number among all cells
    return maximum( data[:,3] )
end     # end of setlastframe function
function setlastcell(data)
    # gets the highest cellid among all cells
    return maximum( data[:,1] )
end     # end of setlastcell function

function getcellindex( lineagetree::Lineagetree, celllistid::Int64 )
    # gives cellid for given celllistid
    return lineagetree.datawd[celllistid,1]
end     # end of getcellindex function
function getcelllistindex( lineagetree::Lineagetree, cellid::Int64 )
    # gives celllistid for given cellid
    celllistids = collect(1:lineagetree.nocells)
    return celllistids[lineagetree.data[:,1].==cellid][1]
end     # end of getcelllistindex function
function getmother( lineagetree::Lineagetree, celllistid::Int64 )
...
end     # end of getmother function
function getdaughters( lineagetree::Lineagetree, celllistid::Int64)
...
end     # end of getdaughters function
function getsister( lineagetree::Lineagetree, celllistid::Int64 )
    # gives sister of celllistid
    # output in order: sisterid, sisterlistid
    motherlistid = getmother(lineagetree,celllistid)[2]
    if( motherlistid==-1 )    # no mother
        return -1,-1
    else                # mother exists
        (sister1id,sister1listid, sister2id,sister2listid) = getdaughters(lineagetree,motherlistid)
        ...
        elseif( sister2listid==celllistid ) # second daughter is input, first one is desired output
            ...
            @printf( " Warning - getsister: Input celllistid is not part of mother's daughters" )
...
end     # end of getsisters function
function getlifedata( lineagetree::Lineagetree, celllistid::Int64 )
    ...
    if( (lineagetree.datawd[celllistid,3]==lineagetree.lastframe) | (lineagetree.unknownfates[celllistid]) )
        ...
        cellfate = (getdaughters(lineagetree,celllistid)[1]!=-1) + 1
    end     # end of getting cellfate
    return lifetime, cellfate
end     # end of getlifedata function
function getlastpreviousframe( lineagetree::Lineagetree, celllistid::Int64 )
    # gives last previous frame that was recorded (not missing); NaN, if before start of recording
    if( getmother( lineagetree, celllistid )[2]<0 )              # check if mother known
        return NaN
    ...
    while( notyetfound )
        ...
        if( lastpreviousframe<lineagetree.firstframe )          # outside of recorded range
            lastpreviousframe = NaN; notyetfound = false
...
end     # end of getlastpreviousframe function
function getfirstnextframe( lineagetree::Lineagetree, celllistid::Int64 )
    # gives first next frame that was recorded (not missing); NaN, if after end of recording
    if( getlifedata( lineagetree, celllistid )[2]<0 )           # check if cellfate known
        return NaN
...
function getclosestcommonancestor( lineagetree::Lineagetree, celllistid1::Int64, celllistid2::Int64 )
    ...
    ancestor1::Int64 = deepcopy(celllistid1);  nogen1::Int64 = 0
    ancestor2::Int64 = deepcopy(celllistid2);  nogen2::Int64 = 0
    while( ancestor1!=ancestor2 )
        if( lineagetree.data[ancestor1,2]<=lineagetree.data[ancestor2,2] )  # ancestor 2 born later
            ancestor2 = getmother(lineagetree,ancestor2)[2]; nogen2 += 1
        else                                                                # ancestor 1 born later
            ancestor1 = getmother(lineagetree,ancestor1)[2]; nogen1 += 1
        end     # end of identifying younger ancestor
        if( (ancestor1==-1) | (ancestor2==-1) )
            ancestor1 = -1; ancestor2 = -1
    ...
    if( ancestor1==-1 )                 # no common ancestor
        ...
        ancestorid = getcellindex(lineagetree,ancestor1)
    end     # end if no common ancestor

    return ancestorid,ancestor1, nogen1,nogen2
end     # end of getclosestcommonancestor function
function getcloserelatives( lineagetree::Lineagetree, celllistid::Int64, depth::UInt64 )
    # outputs a list of all relatives up to the given depth

    depth_here = 0; oldestancestor = deepcopy(celllistid)
    mother = getmother(lineagetree,oldestancestor)[2]
    while( (depth_here<depth) & (mother>0) )        # move upwards the lineage tree to find oldest ancestor within given depth
        depth_here += 1; oldestancestor = deepcopy(mother)
        mother = getmother(lineagetree,oldestancestor)[2]
    ...
    allcells[1] = oldestancestor; j_active = 0; j_cellcounter = 1; j_otherdaughters = 0
    #@printf( " Info - getcloserelatives: celllistid %d, depth %d, oldestancestor %d, depth_here %d, noallcells %d\n", celllistid,depth, oldestancestor,depth_here, noallcells )
    while( j_active<j_cellcounter )
        j_active += 1                               # proceed along cells
        bothdaughters = collect( getdaughters(lineagetree,allcells[j_active])[[2,4]] )
        ...
        for j_cell = bothdaughters[bothdaughters.>0]# only add, if daughters actually exist
            if( sum(getclosestcommonancestor(lineagetree, celllistid,j_cell)[[3,4]])<=depth )   # ie close enough to celllistid
                j_cellcounter += 1; allcells[j_cellcounter] = j_cell
            else                                    # ie too many generations apart
                j_otherdaughters += 1;  allcells[noallcells+1-j_otherdaughters] = j_cell
...
function getcurrentalivenumbers( lineagetree::Lineagetree, currenttime::Float64, onlycomplete::Bool=true )
...
end     # end of getcurrentalivenumbers function
function getcurrentalivenumbers( datawd::Array{Float64,2}, currenttime::Float64, onlycomplete::Bool=true )
    # gets the number of alive cells at given time
    # outputs number of alive cells and celllistids of alive cells
...
function shortenlineagetree( lineagetree::Lineagetree, starttime::Float64,endtime::Float64 )
    ...
    while( lineagetree.missingframes[startframe-lineagetree.firstframe+1] )     # this frame is missing
        ...
        if( startframe>lineagetree.lastframe )                                  # no frames left
            return  initialiseLineagetree(shortname,zeros(Int64,0,4),-1,-1)
    ...
    for j_celllistid = 1:lineagetree.nocells
        ...
        if( shortdata[j_celllistid,3]<shortdata[j_celllistid,2] )               # no visible frame
            ...
            bothdaughters = collect( getdaughters(lineagetree,Int64(j_celllistid))[[2,4]] )
    ...
    return  initialiseLineagetree(shortname,shortdata,shortunknownfates,shortmissingframes)
end     # end of shortenlineagetree function
function getnumberofgenerations( lineagetree::Lineagetree )
    ...
    for j_cell = 1:lineagetree.nocells
        for jj_cell = unknownmothercells
            (ancestor_here, nogen_here) = getclosestcommonancestor( lineagetree, Int64(j_cell), Int64(jj_cell) )[[2,3]]
    ...
    return nogenpercell
end     # end of getnumberofgenerations function
function outputvalues( lineagetree::Lineagetree )
    # outpus all values of lineagetree into the control-window

    @printf( " Info - outputvalues: Lineagetree:\n" )
    @printf( "  name:\t\t%s\n", lineagetree.name )
    @printf( "  nocells:\t%d\n", lineagetree.nocells )
    ...
    for j_cell = 1:lineagetree.nocells
        @printf( "   %4d  %4d %4d  %4d %4d %4d  %4d %4d %4d\n", lineagetree.datawd[j_cell,1],lineagetree.datawd[j_cell,2],lineagetree.datawd[j_cell,3],lineagetree.datawd[j_cell,4],lineagetree.datawd[j_cell,5],lineagetree.datawd[j_cell,6],lineagetree.datawd[j_cell,7],lineagetree.datawd[j_cell,8],lineagetree.datawd[j_cell,9] )
    end     # end of cells loop
    @printf( "  unknownfates:\n  " )
    for j_cell = 1:lineagetree.nocells
    ...
    @printf( "\n" )
    =#
end     # end of outputvalues function
function ploteventtimes( lineagetree::Lineagetree, pars_cell_here::Array{Float64,1}=fill!(Array{Float64,1}(undef,1),NaN) )
    # plots death and division times

    # get auxiliary parameters:
    dthtimes = zeros(lineagetree.nocells); dthcounter = 0
    divtimes = zeros(lineagetree.nocells); divcounter = 0
    inctimes_end = zeros(lineagetree.nocells); inccounter_end = 0           # incomplete end/right-censored
    inctimes_start = zeros(lineagetree.nocells); inccounter_start = 0       # incomplete start/left-cencored
    inctimes_both = zeros(lineagetree.nocells); inccounter_both = 0

    for j_cell = 1:lineagetree.nocells
        (lifetime,cellfate) = getlifedata( lineagetree, Int(j_cell) )
        mother = getmother( lineagetree, Int(j_cell) )[2]
        if( (mother>0) & (cellfate>0) )
            ...
            elseif( cellfate==2 )
                ...
                @printf( " Warning - ploteventtimes: Unknown cellfate %d for cell %d.\n", cellfate,j_cell )
    ...
    minbin = 0.0; maxbin = maximum(vcat(dthtimes,divtimes,inctimes_both,inctimes_end,inctimes_start)); dbin = (maxbin-minbin)/res; mybins = minbin:dbin:maxbin
    ...
    if( !isnan(pars_cell_here[1]) )
        ...
        distr_icp_start = (-beta.*timerange) .+ dthdivdistr.get_logdistr( pars_cell_here, timerange ) .+ log(dt); distr_icp_start = distr_icp_start .- (maximum(distr_icp_start)-600)
        distr_icp_start = cumsum( exp.(distr_icp_start) ); distr_icp_start = log.( distr_icp_start[end] .- distr_icp_start ); distr_icp_start = (beta.*timerange) + distr_icp_start; distr_icp_start .-= logsumexp(distr_icp_start)
    ...
    display(p1)
end     # end of ploteventtimes function
function drawlineagetree( lineagetree::Lineagetree )
    # draws lineagetree
    ...
    for j_cell = orderedcells                       # go through cells in birth-order
        ...
        if( mother<0 )                              # no mother/new branch
            nobranches += 1
            affiliation[2,j_cell] = nobranches; affiliation[3,j_cell] = 0
        else                                        # part of existing branch
            affiliation[2,j_cell] = affiliation[2,mother]; affiliation[3,j_cell] = affiliation[3,mother] + 1
        end     # end if mother exists
    end     # end of cells loop
    ...
    # ...draw nodes:
    for j_cell = (1:lineagetree.nocells)[affiliation[2,:].<=maxbranch]
        cellfate = getlifedata(lineagetree,Int(j_cell))[2]
        if( cellfate==1 )               # death
        ...
        elseif( cellfate==-1 )          # unknown
            ...
            @printf( " Warning - drawlineagetree: Unknown cellfate %d for cell %d\n", cellfate,j_cell )
        end     # end of distinguishing cellfate
    end     # end of cells loop
    display(p1)
    =#
    # regularise:
    tobeminimised = ( x->getlineagetreecost(lineagetree,affiliation, cat(x',pos[2,:]',dims=1) ) )
    #@printf( " Info - drawlineagetree: Start cost: %+1.5e\n", tobeminimised(pos[1,:]) )
    ...
    display(p1)
end     # end of drawlineagetree function
function getlineagetreecost( lineagetree::Lineagetree, affiliation::Array{Int64,2}, pos::Array{Float64,2} )
    ...
    return cost
end     # end of getlineagetreecost funciton

function writelineagetree( lineagetree::Lineagetree, outputfile::String )
    ...
    @printf( " Info - writelineagetree: Try to write\n" )
    @printf( "  %s\n", outputfile )
    ...
    return outputfile
end     # end of writeLineagetree funciton